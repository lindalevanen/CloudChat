...
exports.createUrlPreview = functions.region(region).database.ref('chatEvents/{chatUid}/{messageUid}')
  .onCreate(async (snapshot, context) => {
    ...
    const isUrl = regexUrl.test(messageBody)
    if (!isUrl) {
      console.log("Not an URL")
      return null
    }
    console.log(`${messageBody} seems to be a URL! fetching for preview metadata`)
    const linkPreviewURL = 'http://api.linkpreview.net/?key=' + linkPreviewAPIKey + '&q=' + messageBody
